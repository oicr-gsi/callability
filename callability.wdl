...
task calculateCallability {
  ...
  mosdepth -t ~{threads} --mapq ~{min_map_q} -n --quantize 0:~{normalMinCoverage - 1}: normal ~{normalBam}
  mosdepth -t ~{threads} --mapq ~{min_map_q} -n --quantize 0:~{tumorMinCoverage - 1}: tumor ~{tumorBam}
